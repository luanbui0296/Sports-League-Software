...
public class PlayerInfo extends JFrame {
	...
	private final JLabel lblDesignedByMr = new JLabel("Designed By: Mr. Joshuha S.");
	...
	public void refreshTable(){
		...
		try {
			String query = "select PID, Name, Role, Age from Employeeinfo";
	...
	public PlayerInfo() {
		...
		comboBoxSelect.setModel(new DefaultComboBoxModel(new String[] {"PID", "Name", "Role", "Age"}));
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				try {
					...
					String query = "select * from Employeeinfo where PID = '" + EID + "' ";
					PreparedStatement pst = connection.prepareStatement(query);					
					ResultSet rs = pst.executeQuery();					
					while(rs.next()){
						textFieldEID.setText(rs.getString("PID"));
						textFieldName.setText(rs.getString("Name"));
						textFieldSurname.setText(rs.getString("Role"));
		...
		lblNewLabel = new JLabel("Sports League Management Software");
		...
		btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					String query = "insert into Employeeinfo (PID, Name, Role, Age) values (?, ?, ?, ?) ";
					PreparedStatement pst = connection.prepareStatement(query);					
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					String query = "update Employeeinfo set PID = '" + textFieldEID.getText() + "', Name = '" + textFieldName.getText() + "', Role = '" + textFieldSurname.getText() +"', Age = '" + textFieldAge.getText() + "' where PID = '" + textFieldEID.getText() + "'";
					PreparedStatement pst = connection.prepareStatement(query);									
					pst.execute();				
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(action == 0){
				try {
					String query = "delete from Employeeinfo where PID = '" + textFieldEID.getText() + "' ";
					PreparedStatement pst = connection.prepareStatement(query);
					pst.execute();
		...
		lblNewLabel_3.setBounds(10, 0, 215, 124);
	...
	public void Search(){
		try {
            ...
            String query = "select PID, Name, Role, Age from Employeeinfo where " + selection +" = ? ";
			PreparedStatement pst = connection.prepareStatement(query);
